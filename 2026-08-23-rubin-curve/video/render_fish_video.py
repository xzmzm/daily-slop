#!/usr/bin/env python3
"""Render the Vera Rubin flat rotation-curve story video with Fish Audio's 哈基米 voice.
...
def make_fish_audio(work_dir: Path, api_key: str) -> tuple[Path, list[float]]:
    ...
    return local.assemble_narration(audio_dir, durations)


def main() -> None:
    parser = argparse.ArgumentParser(description="Render the Vera Rubin video with Fish Audio.")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="rubin-fish-build-") as temp_dir_str:
