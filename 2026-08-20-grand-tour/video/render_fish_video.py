#!/usr/bin/env python3
"""Render the Voyager 2 Grand Tour story video with Fish Audio's 哈基米 voice.
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=VIDEO_DIR / "grand-tour-zh-fish.mp4")
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="grand-tour-fish-video-build-", dir=str(VIDEO_DIR)))
    ...
    try:
        ...
        local.assemble_video(frames_dir, narration, subtitles, args.output)
        print(f"rendered Fish video -> {args.output}")
    finally:
        server.terminate()
        server.wait()
    ...
    metadata = {
        "slug": local.SLUG,
        "title": "Voyager 2 Grand Tour · Gravity Assist Simulator",
        "voice": "Fish Audio / 哈基米 (s2.1-pro-free)",
        "model": "Gemini 3.7 Flash",
        "durations": durations,
        "total_duration": local.duration(args.output),
    }
    meta_path = args.output.with_suffix(".json")
    meta_path.write_text(json.dumps(metadata, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(f"wrote metadata -> {meta_path}")

    # Remove temporary work directory safely
    subprocess.run(["trash", str(work_dir)], check=True)
