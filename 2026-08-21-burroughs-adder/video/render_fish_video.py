#!/usr/bin/env python3
"""Render the 1888 Burroughs Adding Machine story video with Fish Audio's 哈基米 voice.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render 1888 Burroughs Adding Machine video with Fish Audio.")
    ...
    if not api_key:
        print("FISH_AUDIO_API_KEY not found in environment or workspace .env. Falling back to local TTS.", file=sys.stderr)
        local.main()
        return
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="burroughs-fish-build-") as temp_dir_str:
